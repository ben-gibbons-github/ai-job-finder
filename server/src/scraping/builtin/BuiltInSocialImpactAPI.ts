      ...
      typeof companyObj.name === 'string'
        ...
        : typeof companyObj.legalName === 'string'
          ...
          : typeof companyObj.alternateName === 'string'
            ...
            : '';
    if (nestedCompanyName.trim()) {
      return nestedCompanyName.trim();
    }
  }
...
export function collectBuiltInCompanyJobPairsFromHtml(html: string): NormalizedPortalJob[] {
...
export async function fetchAllBuiltInSocialImpactJobs(): Promise<ScrapedJob[]> {
  try {
    const normalized: NormalizedPortalJob[] = [];

    for (let page = 1; page <= MAX_BUILTIN_SOCIAL_IMPACT_PAGES; page += 1) {
      const url = page === 1 ? BUILTIN_SOCIAL_IMPACT_JOBS_URL : `${BUILTIN_SOCIAL_IMPACT_JOBS_URL}?page=${page}`;
    ...
    return normalizeJobsWithCoordinates('BuiltInSocialImpact', hydrated);
  } catch (error) {
