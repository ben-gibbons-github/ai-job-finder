...
export function parseHealthECareersJobs(html: string): NormalizedPortalJob[] {
...
async function hydrateHealthECareersDescriptions(jobs: NormalizedPortalJob[]): Promise<NormalizedPortalJob[]> {
...
export async function fetchAllHealthECareersJobs(): Promise<ScrapedJob[]> {
  try {
    ...
    const hydrated = await hydrateHealthECareersDescriptions(normalized);
    return normalizeJobsWithCoordinates('HealthECareers', hydrated);
  } catch (error) {
