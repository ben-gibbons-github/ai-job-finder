...
const WWR_PRODUCT_BASE_URL = 'https://weworkremotely.com/categories/remote-product-jobs';
const MAX_WWR_PRODUCT_PAGES = 400;

function pageUrl(page: number): string {
  const url = new URL(WWR_PRODUCT_BASE_URL);
  if (page > 1) {
...
function parseWeWorkRemotelyProductJobs(html: string): NormalizedPortalJob[] {
  ...
  for (const match of html.matchAll(linkPattern)) {
    ...
    jobs.push({
      ...
      sourceUrl,
      description: '',
      tags: ['WeWorkRemotely', 'Product', 'Remote'],
    });
  }
...
export async function fetchAllWeWorkRemotelyProductJobs(): Promise<ScrapedJob[]> {
  try {
    const normalized = await collectPaginatedHtmlJobs({
      sourceName: 'WeWorkRemotelyProduct',
      maxPages: MAX_WWR_PRODUCT_PAGES,
      pageUrl,
      parseJobs: (html) => parseWeWorkRemotelyProductJobs(html),
    });

    return normalizeJobsWithCoordinates('WeWorkRemotelyProduct', normalized);
  } catch (error) {
    console.warn('[WeWorkRemotelyProductAPI] Failed to fetch jobs:', String(error));
    return [];
  }
}
