...
describe('NameToLonLat', () => {
    ...
    describe('nameToLonLat', () => {
        ...
        it('should throw error when location not found', async () => {
            ...
            await expect(nameToLonLat('InvalidLocation123')).rejects.toThrow('Failed to geocode location "InvalidLocation123" with all providers: No results found for location: InvalidLocation123');
        });
        it('should throw error on API failure', async () => {
            strategyMock.mockRejectedValueOnce(new Error('Network error'));
            await expect(nameToLonLat('AnyLocation')).rejects.toThrow('Failed to geocode location "AnyLocation" with all providers: Network error');
        });
    });
