...
const WWR_CUSTOMER_SUPPORT_BASE_URL = 'https://weworkremotely.com/categories/remote-customer-support-jobs';
const MAX_WWR_CUSTOMER_SUPPORT_PAGES = 400;

function pageUrl(page: number): string {
  const url = new URL(WWR_CUSTOMER_SUPPORT_BASE_URL);
  if (page > 1) {
...
function parseWeWorkRemotelyCustomerSupportJobs(html: string): NormalizedPortalJob[] {
  ...
  for (const match of html.matchAll(linkPattern)) {
    ...
    jobs.push({
      ...
      sourceUrl,
      description: '',
      tags: ['WeWorkRemotely', 'Customer Support', 'Remote'],
    });
  }
...
export async function fetchAllWeWorkRemotelyCustomerSupportJobs(): Promise<ScrapedJob[]> {
  try {
    const normalized = await collectPaginatedHtmlJobs({
      sourceName: 'WeWorkRemotelyCustomerSupport',
      maxPages: MAX_WWR_CUSTOMER_SUPPORT_PAGES,
      pageUrl,
      parseJobs: (html) => parseWeWorkRemotelyCustomerSupportJobs(html),
    });

    return normalizeJobsWithCoordinates('WeWorkRemotelyCustomerSupport', normalized);
  } catch (error) {
    console.warn('[WeWorkRemotelyCustomerSupportAPI] Failed to fetch jobs:', String(error));
    return [];
  }
}
