...
const MEDICAL_JOBS_RSS_URLS = [
  'https://www.indeed.com/rss?q=medical+assistant&l=United+States',
  'https://www.indeed.com/rss?q=registered+nurse&l=United+States',
  'https://www.indeed.com/rss?q=phlebotomist&l=United+States',
  'https://www.indeed.com/rss?q=medical+records&l=United+States',
];
...
function splitTitleParts(title: string): { role: string; company: string } {
  ...
  return { role: title, company: 'Medical Employer' };
}

function parseMedicalJobsFromRss(xml: string): NormalizedPortalJob[] {
  ...
  for (const match of xml.matchAll(itemPattern)) {
    ...
    const isRemote = /remote|telehealth|work from home|wfh/i.test(`${rawTitle} ${description}`);
    ...
    jobs.push({
      ...
      description,
      tags: ['MedicalJobs', 'Healthcare', 'Clinical', 'Medical'],
    });
  }
...
export async function fetchAllMedicalJobs(): Promise<ScrapedJob[]> {
  try {
    const normalized: NormalizedPortalJob[] = [];

    for (const url of MEDICAL_JOBS_RSS_URLS) {
      ...
      const rows = parseMedicalJobsFromRss(xml);
    ...
    return normalizeJobsWithCoordinates('MedicalJobs', Array.from(dedup.values()));
  } catch (error) {
    console.warn('[MedicalJobsAPI] Failed to fetch jobs:', String(error));
    return [];
  }
}
