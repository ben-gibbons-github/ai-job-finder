...
import type { TagCloudEntry } from './server/TagCloud.js'
...
// Global job list
let JOBS: ScrapedJob[] = []
let SCRAPE_LOAD_DEBUG_STATS: ScrapeLoadDebugStats | null = null
let DEBUG_COVERAGE_STATS: ServerDebugCoverageStats | null = null
const searchMain = new SearchMain()
const top100Search = new Top100Search(searchMain)

function recomputeDebugCoverageStats(): ServerDebugCoverageStats | null {
...
onBackgroundGeocodeComplete(() => {
  ...
  console.log(
    `[DebugCoverage] Refreshed after background geocode: geocoded=${liveCoverage.geocodedCount}/${JOBS.length} (${liveCoverage.geocodedPct}%)`,
  )
})
