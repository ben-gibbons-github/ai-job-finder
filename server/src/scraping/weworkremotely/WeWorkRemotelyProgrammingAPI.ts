...
function extractWeWorkRemotelyDescription(context: string): string {
  const descriptionMatches = Array.from(context.matchAll(/<description[^>]*>([\s\S]*?)<\/description>/gi))
  ...
  const summaryMatch = context.match(/<\s*(?:p|li|div)[^>]*>([\s\S]*?)<\s*\/\s*(?:p|li|div)>/i);
  if (summaryMatch) {
    const summary = sanitizeJobDescription(summaryMatch[1] || '');
...
function parseWeWorkRemotelyProgrammingJobs(html: string): NormalizedPortalJob[] {
  ...
  for (const match of html.matchAll(linkPattern)) {
    ...
    jobs.push({
      ...
      sourceUrl,
      description: extractWeWorkRemotelyDescription(context),
