...
const DEFAULT_MAX_GREENHOUSE_BOARDS = 10_000;
// Greenhouse omits job descriptions unless content=true is passed, so this must default to true.
const DEFAULT_GREENHOUSE_INCLUDE_CONTENT = true;
...
export async function fetchAllGreenhouseJobs(): Promise<ScrapedJob[]> {
  ...
  const boards = Array.from(new Set([
    ...(envBoards.length > 0 ? envBoards : DEFAULT_GREENHOUSE_BOARDS),
    ...getDiscoveredCompanySlugs('greenhouse'),
  ]));
  ...
  const includeContentEnv = String(process.env.GREENHOUSE_INCLUDE_CONTENT || '').trim().toLowerCase();
  const includeContent = includeContentEnv === '' ? DEFAULT_GREENHOUSE_INCLUDE_CONTENT : includeContentEnv === 'true';
