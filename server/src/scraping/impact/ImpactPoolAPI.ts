...
export function extractImpactPoolDescriptionFromCardHtml(cardHtml: string): string {
  const directMatches = Array.from(cardHtml.matchAll(/<[^>]+(?:type=["']body["']|class=["'][^"']*rich-text[^"']*["'])[^>]*>([\s\S]*?)<\/[^>]+>/gi))
    .map((match) => sanitizeJobDescription(match[1] || ''))
    .filter(Boolean);
  ...
  const bodyContent = Array.from(cardHtml.matchAll(/<(?:p|li|div)[^>]*>([\s\S]*?)(?:<\/p>|<\/li>|<\/div>)/gi))
    .map((match) => sanitizeJobDescription(match[1] || ''))
  ...
  return extractDescriptionFromHtml(cardHtml, '', 1200);
}
...
export function parseImpactPoolJobs(html: string): NormalizedPortalJob[] {
  ...
  for (const match of html.matchAll(linkPattern)) {
    ...
    const description = extractImpactPoolDescriptionFromCardHtml(anchorHtml);
