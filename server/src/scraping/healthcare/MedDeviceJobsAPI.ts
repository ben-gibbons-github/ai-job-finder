...
async function hydrateMedDeviceDescriptions(jobs: NormalizedPortalJob[]): Promise<NormalizedPortalJob[]> {
...
export async function fetchAllMedDeviceJobs(): Promise<ScrapedJob[]> {
  try {
    ...
    const hydrated = await hydrateMedDeviceDescriptions(normalized);
    return normalizeJobsWithCoordinates('MedDeviceJobs', hydrated);
  } catch (error) {
