...
function isHrRole(job: RemoteOkJob): boolean {
  ...
  return /human resources|hr\b|recruiter|talent acquisition|people ops|people operations|people partner/.test(haystack);
}

export async function fetchAllRemoteOkHrJobs(): Promise<ScrapedJob[]> {
  try {
    ...
    const normalized = jobs
      ...
      .filter((job) => Boolean(job.url) && isHrRole(job))
      .map((job) => parseRemoteOkJob(job));

    return normalizeJobsWithCoordinates('RemoteOKHR', normalized);
  } catch (error) {
    console.warn('[RemoteOKHRAPI] Failed to fetch jobs:', String(error));
    return [];
  }
}
