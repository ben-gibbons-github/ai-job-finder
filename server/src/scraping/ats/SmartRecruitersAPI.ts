...
const DEFAULT_SR_MAX_COMPANIES = Number(process.env.SR_MAX_COMPANIES) || 2_000
const DEFAULT_SR_MAX_DETAIL_FETCHES = 50
...
async function hydrateSmartRecruiterDescription(company: string, job: SrJob): Promise<string> {
  ...
  const payload = await fetchJson(detailUrl) as { jobAd?: { sections?: Record<string, string> }; content?: string; description?: string };
  const directDescription = typeof payload?.jobAd?.sections?.jobDescription === 'string'
    ? payload.jobAd.sections.jobDescription
  ...
  if (directDescription && directDescription.trim().length > 40) {
    return directDescription.trim();
  }

  const html = await fetchHtml(String(job.ref ?? detailUrl));
  if (!html) {
    return '';
  }

  return extractDescriptionFromHtml(html, String(job.name ?? ''), 2000);
}
...
export async function fetchAllSmartRecruitersJobs(): Promise<ScrapedJob[]> {
  ...
  const companies = Array.from(new Set([
    ...(envCompanies.length > 0 ? envCompanies : DEFAULT_SR_COMPANIES),
    ...getDiscoveredCompanySlugs('smartrecruiters'),
  ]))
    .slice(0, DEFAULT_SR_MAX_COMPANIES)
  let remainingDetailFetches = Math.max(0, Number(process.env.SR_MAX_DETAIL_FETCHES || DEFAULT_SR_MAX_DETAIL_FETCHES))
  ...
  for (const company of companies) {
    try {
      ...
      for (const job of jobs) {
        ...
        if (!mapped) continue
        const description = remainingDetailFetches > 0
          ? await hydrateSmartRecruiterDescription(company, job)
          : ''
        if (remainingDetailFetches > 0) remainingDetailFetches -= 1
        normalized.push({ ...mapped, description })
      }
