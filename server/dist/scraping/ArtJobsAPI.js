...
function extractDetailFields(detailHtml, fallbackTitle) {
    ...
    const locationCandidate = nonTitleBlocks.find((block) => /(?:^|\s)(?:Remote|Hybrid|[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*,\s*[A-Z]{2}|[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\s*,\s*[A-Z]{2})\b/.test(block)
        || /(?:United States|Canada|UK|Europe)/i.test(block)) || 'Unknown';
    const companyCandidate = nonTitleBlocks.find((block) => block !== locationCandidate
        && !/remote|hybrid|design|gallery|community|creative|art|job|jobs|location|full[- ]?time|artist/i.test(block)
        && block.length > 2) || 'ArtJobs';
    const descriptionBlocks = nonTitleBlocks.filter((block) => block !== companyCandidate
...
function parseArtJobsPage(html) {
...
async function fetchArtJobsPage(page) {
