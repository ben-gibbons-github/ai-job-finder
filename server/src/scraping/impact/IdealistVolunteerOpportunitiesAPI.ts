...
async function hydrateIdealistVolunteerDescriptions(jobs: NormalizedPortalJob[]): Promise<NormalizedPortalJob[]> {
...
export async function fetchAllIdealistVolunteerOpportunities(): Promise<ScrapedJob[]> {
  try {
    ...
    const hydrated = await hydrateIdealistVolunteerDescriptions(normalized);
    return normalizeJobsWithCoordinates('IdealistVolunteerOpportunities', hydrated);
  } catch (error) {
