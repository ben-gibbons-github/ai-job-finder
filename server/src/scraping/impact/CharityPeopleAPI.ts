...
async function hydrateCharityPeopleDescriptions(jobs: NormalizedPortalJob[]): Promise<NormalizedPortalJob[]> {
...
export async function fetchAllCharityPeopleJobs(): Promise<ScrapedJob[]> {
  try {
    ...
    const hydrated = await hydrateCharityPeopleDescriptions(normalized);
    return normalizeJobsWithCoordinates('CharityPeople', hydrated);
  } catch (error) {
