...
import { fetchJson, normalizeJobsWithCoordinates, type NormalizedPortalJob } from '../core/PortalIngestionUtils.js';
...
const DEFAULT_ASHBY_MAX_CONCURRENCY = 12;
...
function formatAshbyCompensation(value: unknown): string {
  if (!value || typeof value !== 'object') {
    return '';
  }
  ...
  const parts = [
    ...
    compensation.minValue ?? compensation.min,
    compensation.maxValue ?? compensation.max,
    compensation.interval,
  ].map((part) => String(part ?? '').trim()).filter(Boolean);
  return parts.length > 0 ? `Compensation: ${parts.join(' ')}` : 'Compensation included.';
}

async function fetchAshbyOrgsWithConcurrency(orgs: string[]): Promise<NormalizedPortalJob[]> {
  const maxConcurrency = Math.max(1, Number(process.env.ASHBY_MAX_CONCURRENCY || DEFAULT_ASHBY_MAX_CONCURRENCY));
  const results: NormalizedPortalJob[] = [];
  let nextIndex = 0;

  await Promise.all(Array.from({ length: Math.min(maxConcurrency, orgs.length) }, async () => {
    while (nextIndex < orgs.length) {
      const org = orgs[nextIndex++];
      results.push(...await fetchAshbyOrgJobs(org));
    }
  }));
...
async function fetchAshbyOrgJobs(org: string): Promise<NormalizedPortalJob[]> {
  try {
    ...
    return jobs.flatMap((job) => {
      ...
      if (!id || !title) return [];
      const location = String(job.locationName ?? job.location ?? 'Unknown').trim() || 'Unknown';
      const department = String(job.departmentName ?? job.department ?? '').trim();
      const compensation = formatAshbyCompensation(job.compensation);
      return [{
        title,
        company: org,
        location,
        remote: /remote/i.test(location) ? 'Remote' : 'Unknown',
        type: String(job.employmentType ?? 'Unknown'),
        sourceUrl: String(job.applyUrl ?? `https://jobs.ashbyhq.com/${org}/job/${id}`),
        posted: typeof job.publishedAt === 'string' ? job.publishedAt : undefined,
        description: [compensation, String(job.descriptionPlain ?? job.descriptionHtml ?? '')].filter(Boolean).join('\n\n'),
        tags: ['Ashby', department, compensation].filter(Boolean),
      }];
    });
  } catch {
    try {
      const response = await scraperFetch(`https://jobs.ashbyhq.com/${org}`, {
        method: 'GET', signal: AbortSignal.timeout(30_000), headers: { Accept: 'text/html,application/xhtml+xml' },
      });
      return response.ok ? parseAshbyEmbeddedData(org, await response.text()) : [];
    } catch {
      return [];
    }
  }
...
export async function fetchAllAshbyJobs(): Promise<ScrapedJob[]> {
  ...
  const orgs = Array.from(new Set([
    ...(envOrgs.length > 0 ? envOrgs : DEFAULT_ASHBY_ORGS),
    ...getDiscoveredCompanySlugs('ashby'),
  ]));

  const normalized = await fetchAshbyOrgsWithConcurrency(orgs);
