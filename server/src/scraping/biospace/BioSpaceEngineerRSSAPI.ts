...
const BIOSPACE_ENGINEER_RSS_URL = 'https://jobs.biospace.com/jobsrss/?keywords=engineer';
...
function parseBioSpaceEngineerRss(xml: string): NormalizedPortalJob[] {
  ...
  for (const match of xml.matchAll(itemPattern)) {
    ...
    jobs.push({
      title,
      company: 'BioSpace Engineer Employer',
      location: 'Unknown',
      ...
      posted,
      description,
      tags: ['BioSpace', 'Biotech', 'Engineer', 'Medical Tech'],
    });
  }
  return jobs;
}

export async function fetchAllBioSpaceEngineerRssJobs(): Promise<ScrapedJob[]> {
  try {
    const response = await scraperFetch(BIOSPACE_ENGINEER_RSS_URL, {
      method: 'GET',
    ...
    const normalized = parseBioSpaceEngineerRss(xml);
    const dedup = new Map<string, NormalizedPortalJob>();
    for (const row of normalized) dedup.set(row.sourceUrl, row);
    return normalizeJobsWithCoordinates('BioSpaceEngineerRSS', Array.from(dedup.values()));
  } catch (error) {
    console.warn('[BioSpaceEngineerRSSAPI] Failed to fetch jobs:', String(error));
    return [];
  }
}
