...
const PHARMIWEB_DATA_RSS_URL = 'https://www.pharmiweb.jobs/jobsrss/?keywords=data';
...
function parsePharmiwebDataRss(xml: string): NormalizedPortalJob[] {
  ...
  for (const match of xml.matchAll(itemPattern)) {
    ...
    jobs.push({
      title,
      company: 'Pharmiweb Data Employer',
      location: 'Unknown',
      ...
      posted,
      description,
      tags: ['Pharmiweb', 'Pharma', 'Data', 'Medical Tech'],
    });
  }
  return jobs;
}

export async function fetchAllPharmiwebDataRssJobs(): Promise<ScrapedJob[]> {
  try {
    const response = await scraperFetch(PHARMIWEB_DATA_RSS_URL, {
      method: 'GET',
    ...
    const normalized = parsePharmiwebDataRss(xml);
    const dedup = new Map<string, NormalizedPortalJob>();
    for (const row of normalized) dedup.set(row.sourceUrl, row);
    return normalizeJobsWithCoordinates('PharmiwebDataRSS', Array.from(dedup.values()));
  } catch (error) {
    console.warn('[PharmiwebDataRSSAPI] Failed to fetch jobs:', String(error));
    return [];
  }
}
