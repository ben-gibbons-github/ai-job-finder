...
export async function impactJobAIAsync(job, shouldLog = false) {
    const current = getCurrentCompanyAiScores(job);
    if (current.impactSummary.trim().length > 0 || current.impactScore > 0) {
        if (shouldLog) {
            console.log(`[SearchImpactAI] Using cached impact for company=${String(job.company_name ?? '').trim() || '?'} title=${String(job.name ?? '').trim() || '?'} score=${clampToPercent(current.impactScore)}`);
        }
        return {
    ...
    if (shouldLog) {
        console.log(`[SearchImpactAI] Requesting unified impact for company=${String(job.company_name ?? '').trim() || '?'} title=${String(job.name ?? '').trim() || '?'} ` +
