...
export function parseJmirCareersJobs(html: string): NormalizedPortalJob[] {
  ...
  for (const match of html.matchAll(linkPattern)) {
    ...
    jobs.push({
      title,
      company: company || 'JMIR Careers Employer',
...
async function hydrateJmirDescriptions(jobs: NormalizedPortalJob[]): Promise<NormalizedPortalJob[]> {
...
export async function fetchAllJmirCareersJobs(): Promise<ScrapedJob[]> {
  try {
    ...
    const hydrated = await hydrateJmirDescriptions(normalized);
    return normalizeJobsWithCoordinates('JMIRCareers', hydrated);
  } catch (error) {
