...
export async function fetchAllBuiltInHealthTechJobs(): Promise<ScrapedJob[]> {
  try {
    const normalized: NormalizedPortalJob[] = [];

    for (let page = 1; page <= MAX_BUILTIN_HEALTHTECH_PAGES; page += 1) {
      const url = page === 1 ? BUILTIN_HEALTHTECH_JOBS_URL : `${BUILTIN_HEALTHTECH_JOBS_URL}?page=${page}`;
    ...
    return normalizeJobsWithCoordinates('BuiltInHealthTech', hydrated);
  } catch (error) {
    console.warn('[BuiltInHealthTechAPI] Failed to fetch jobs:', String(error));
    return [];
  }
}
