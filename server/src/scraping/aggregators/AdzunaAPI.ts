...
import { ALL_SCRAPE_COUNTRY_CODES } from '../core/SharedCountries.js';
...
function isAdzunaAuthFailure(
  payload: AdzunaResponse | null,
  status?: number,
  rawText?: string,
): boolean {
  ...
  if (
    payload?.exception === 'AUTH_FAIL' ||
    payload?.display === 'Authorisation failed' ||
    payload?.__CLASS__ === 'Adzuna::API::Response::Exception' ||
    (typeof payload?.display === 'string' && /authoris?ation failed/i.test(payload.display)) ||
    (typeof payload?.exception === 'string' && /auth_fail/i.test(payload.exception))
  ) {
    return true;
  }
  if (rawText && (rawText.includes('AUTH_FAIL') || /authoris?ation failed/i.test(rawText))) {
    return true;
  }
  return false;
}
...
async function fetchAdzunaPage(
  ...
  if (now < adzunaCooldownUntilMs) {
    return { jobs: [], rateLimited: true, unsupportedCountry: false, skippedCachedPage: false, httpCache: 'miss', authFailed: false };
  }
  ...
  try {
    ...
    const httpCache = response.fromCache ? 'hit' : 'miss';
    const sourceLabel = httpCache === 'hit' ? 'cached' : 'fresh';
    ...
    if (!response.ok) {
      if (response.status === 429) {
        ...
        return { jobs: [], rateLimited: true, unsupportedCountry: false, skippedCachedPage: false, httpCache, authFailed: false };
      }
      ...
      if (response.status === 404) {
        ...
        return { jobs: [], rateLimited: false, unsupportedCountry: true, skippedCachedPage: false, httpCache, authFailed: false };
      }

      const rawText = await response.text().catch(() => '');

      let errPayload: AdzunaResponse | null = null;
      try {
        errPayload = rawText ? (JSON.parse(rawText) as AdzunaResponse) : null;
      } catch {
        errPayload = null;
      }

      if (isAdzunaAuthFailure(errPayload, response.status, rawText)) {
        console.warn(
          `[AdzunaAPI] Authorization failed (${sourceLabel} page, country=${country}, keyword="${keyword}", page=${page}): ${errPayload?.display || errPayload?.exception || response.statusText || 'Authorisation failed'}`,
        );
        return { jobs: [], rateLimited: false, unsupportedCountry: false, skippedCachedPage: false, httpCache, authFailed: true };
      }
    ...
    const rawText = await response.text().catch(() => '');
    console.log(`[AdzunaAPI] Raw response text: ${rawText}`);

    let payload: AdzunaResponse | null = null;
    try {
      payload = rawText ? (JSON.parse(rawText) as AdzunaResponse) : null;
    } catch {
      payload = null;
    }

    if (isAdzunaAuthFailure(payload, response.status, rawText)) {
      console.warn(
        `[AdzunaAPI] Authorization failed (${sourceLabel} page, country=${country}, keyword="${keyword}", page=${page}): ${payload?.display || payload?.exception || 'Authorisation failed'}`,
      );
      return { jobs: [], rateLimited: false, unsupportedCountry: false, skippedCachedPage: false, httpCache, authFailed: true };
    }

    if (httpCache === 'hit' && shouldSkipParsingCachedHttpPages()) {
      // add some logging here:
      console.log(`[AdzunaAPI] Skipping parsing cached page for country=${country} keyword="${keyword}" page=${page} httpCache=${httpCache}`);
      return { jobs: [], rateLimited: false, unsupportedCountry: false, skippedCachedPage: true, httpCache, authFailed: false };
    }

    const rows = Array.isArray(payload?.results) ? payload.results : [];
    // console.log(
    //   `[AdzunaAPI] Successful response (${sourceLabel} page, country=${country}, keyword="${keyword}", page=${page}): ${rows.join('\n')} \njob(s).`,
    // );
    return {
      jobs: rows
      ...
      skippedCachedPage: false,
      httpCache,
      authFailed: false,
    };
  } catch (error) {
    if (isRateLimitedScrapeError(error)) {
      console.warn('[AdzunaAPI] Rate limited by Adzuna; stopping the entire Adzuna scrape.');
      return { jobs: [], rateLimited: true, unsupportedCountry: false, skippedCachedPage: false, httpCache: 'miss', authFailed: false };
    }

    console.warn(`[AdzunaAPI] Failed country=${country} keyword="${keyword}" page=${page}:`, String(error));
    return { jobs: [], rateLimited: false, unsupportedCountry: false, skippedCachedPage: false, httpCache: 'miss', authFailed: false };
  }
}

export async function fetchAllAdzunaJobs(onPageJobs?: (jobs: ScrapedJob[]) => Promise<void>): Promise<ScrapedJob[]> {
  ...
  for (const country of usedCountries) {
    ...
    for (const keyword of usedKeywords) {
      ...
      for (let page = 1; page <= maxPages; page += 1) {
        ...
        if (onPageJobs) {
          await onPageJobs(await normalizeJobsWithCoordinates('Adzuna', jobs));
        }
      }
    }
  }
